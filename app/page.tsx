...
import { USERNAME_HINT, PASSWORD_HINT, generatePassword } from '@/lib/credentials'
...
function TeachersTab({
  ...
  async function addTeacher(e: React.FormEvent) {
    ...
    if (!res.ok) {
      setCredMessage(data.error ?? `Қате шықты (код ${res.status})`)
      return
    }

    if (username.trim() && password) {
      setCreds([
        { id: data.id, name, subject: subject || null, username: data.username ?? username, password },
      ])
    }
    setName('')
    setSubject('')
        ...
        {teachers.map((t) => (
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {t.username ? 'Кездейсоқ пароль' : 'Автоматты беру'}
              </button>
              <button
...
function TeacherLoginForm({
  ...
  teacher: Teacher
  onDone: (issued: Credential | null) => Promise<void>
}) {
  ...
  async function save(e: React.FormEvent) {
    ...
    // If a new password was typed, show it once so the admin can copy it to the teacher.
    await onDone(
      password
        ? {
            id: teacher.id,
    ...
    <form onSubmit={save} className="mt-3 space-y-2 rounded-xl border border-zinc-200 bg-zinc-50 p-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className="rounded-lg border border-zinc-200 bg-white px-3 py-1.5 text-xs text-zinc-600 hover:border-zinc-400"
        >
          Кездейсоқ
        </button>
        <button
